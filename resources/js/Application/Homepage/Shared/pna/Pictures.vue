    <template>
        <Layout>
            ...
            <div v-if="ocont?.id" class="p-4 bg-layout-sun-200 dark:bg-layout-night-200">
                ...
                <h1 class="text-2xl font-bold">{{ decodeEntities(ucf(ocont?.slug)) }}</h1>
    ...
    import {stripTags,remBrackets,ucf} from "@/helpers";
    ...
    //     console.log('DEBUG: scrolling to element', el);
        ...
        (typeof window !== 'undefined' ? window.pageYOffset : 0) - 134;
            window.scrollTo({ top: y, behavior: 'smooth' });
