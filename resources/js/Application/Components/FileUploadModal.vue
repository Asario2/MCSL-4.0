<template>
  <div
    v-if="isModalOpen"
    class="fixed inset-0 flex items-start justify-center bg-black bg-opacity-50 pt-[160px] overflow-y-auto mb-[50px]"
    style="z-index:1000;"
  >
    <div class="bg-layout-sun-100 dark:bg-layout-night-100 rounded-lg shadow-lg w-full max-w-lg p-6 max-h-[calc(100vh-200px)]">
      ...
      <div v-if="!pxa">
        <form @submit.prevent="uploadImage">
          <h3 class="text-2xl font-semibold text-center mb-4">Datei hochladen</h3>
          ...
          >
            ...
            <p class="text-layout-sun-700 dark:text-layout-night-700">
              Ziehe Dateien hierher oder klicke, um Dateien auszuwählen.
            </p>
          </div>
...
export default {
  ...
  methods: {
    ...
    async uploadImage() {
      ...
      } else if (file.name.endsWith('.csv2')) {
        formData.append('csv', file);
      } else {
        formData.append('ofile', file); // PDF oder MP3
      }
      ...
      try {
        const res = await axios.post(`/upload-ofile/${this.tablex}`, formData, {
       ...
       if (file.type === 'text/csv' || file.name.endsWith('.csv')) {
        ...
        console.error("Upload fehlgeschlagen", err.response?.data || err);
        alert("Upload fehlgeschlagen: " + (err.response?.data?.error || 'Unbekannter Fehler'));
        this.uploading = false;
      }
    },
