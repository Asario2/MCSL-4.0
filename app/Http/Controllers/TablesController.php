...
class TablesController extends Controller
{
    ...
    function EditTables(Request $request,$id = '',$table='blogs',$kk=''){
        ...
        if(!CheckRights(Auth::id(),$table,"edit") && $id != '0')
        {
            header("Location: /no-rights");

            exit;
        }
        if($id != 0 && in_array($table,Settings::$private_tables) && Auth::user()->users_rights_id != '1')
        {
            header("Location: /no-rights");
            exit;
        }
...
//             \Log::info(DB::getQueryLog());
    ...
    {
        $db = DB::table("admin_table")->where("name",$table)->value("dbase");
        if(empty($db))
        {
            return $table;
        }
    ...
    public function updateGroup(Request $request, Contact $contact)
    {
        $request->validate([
            'Gruppe' => 'required|string|in:Kunde,Friends,Projects,Offiziell,Familie',
        ]);

        $contact->Gruppe = $request->Gruppe;
        $contact->save();

        return response()->json([
            'success' => true,
            'contact_id' => $contact->id,
            'new_group' => $contact->Gruppe,
        ]);
...
public function ShowTable(Request $request, $table_alt = null)
{
    ...
    elseif($table == "ausgaben"){
        $query->orderBy('plus_minus', 'DESC')
              ->orderBy('cur_amount', 'DESC');
        $order_alt = true;
    }
    else {
        $ord = ["id", "DESC"];
    ...
    public function emailmod()
{
    ...
    foreach ($contacts as $val) {
        $data[] = [
            "id" => $val->id,
            "name" => decval_user($val->Name,Auth::id()) . " (" . decval_user($val->Email,Auth::id()) . ")",
        ];
    }
...
return Inertia::render('Admin/Kontakte', [
    ...
    {
        $slug = $request->table . "_help_" . strtolower($request->slug);
        // \Log::info("SLUG;",[$slug]);
        $res = DB::table("texts")
            ->select("headline", "text as textbody")
            ->where("named_id", $slug)
            ->first();

        return response()->json($res ?? ['headline' => '', 'textbody' => '']);
    }
    ...
    private function safeDecval($value)
    {
        ...
        try {
            return decval_user($value,Auth::id());
        } catch (\Exception $e) {
    ...
    public function StoreTable(Request $request, $table)
    {
        ...
        if($table == "contacts" && !$formData['xis_public_con'])
        {
            ...
            $formData['Name']      = !empty($formData['Name'])
                ? encval_user($formData['Name'], $userId)
                : null;

            $formData['Vorname']      = !empty($formData['Vorname'])
                ? encval_user($formData['Vorname'], $userId)
                : null;

            $formData['Nachname']     = !empty($formData['Nachname'])
                ? encval_user($formData['Nachname'], $userId)
                : null;

            $formData['Email']        = !empty($formData['Email'])
                ? encval_user($formData['Email'], $userId)
                : null;

            $formData['Telefon']      = !empty($formData['Telefon'])
                ? encval_user($formData['Telefon'], $userId)
                : null;

            $formData['Handy']        = !empty($formData['Handy'])
                ? encval_user($formData['Handy'], $userId)
                : null;

            $formData['Strasse']      = !empty($formData['Strasse'])
                ? encval_user($formData['Strasse'], $userId)
                : null;

            $formData['Plz']          = !empty($formData['Plz'])
                ? encval_user($formData['Plz'], $userId)
                : null;

            $formData['Geburtsdatum'] = !empty($formData['Geburtsdatum'])
                ? encval_user($formData['Geburtsdatum'], $userId)
                : null;

            $formData['Kommentar']    = !empty($formData['Kommentar'])
                ? encval_user($formData['Kommentar'], $userId)
                : null;
