...
class HomeController extends Controller
{
    ...
    $res = DB::table('ausgaben')->where("users_id",Auth::id())->where("pub","1")
        ...
        SUM(CASE WHEN plus_minus = 'Einnahme'
            ...
            ELSE 0 END) as res_in,

        SUM(CASE WHEN plus_minus = 'Ausgabe'
            THEN CAST(cur_amount AS DECIMAL(15,2))
            ELSE 0 END) as res_out,
        ...
        SUM(CASE WHEN plus_minus = 'Ausgabe'
            THEN CAST(cur_amount AS DECIMAL(15,2))
            ELSE 0 END) as res_total
    ")
    ->first();
    $resLocal = collect((array)$res)->map(function($value) {
    return number_format($value, 2, ',', '.');
    })->toArray();
        return Inertia::render('Admin/Ausgaben', [
    ...
    public function home_publication(Request $request)
{
   ...
   // $links = $tables->links()->elements[0]; // Array mit url, label, active
    ...
    function home_pna()
    {
        $data = DB::table("texts")->select('texts.*','users.name as author_name')->leftJoin('users', 'users.id', '=', 'texts.users_id')->where("texts.autoslug","y7kw")->first();
        //$data->text = Str::markdown($data->text);

    return Inertia::render('Homepage/pna/home_pna',["data" => [$data]]);
    }
...
//          \Log::info("author_img: ".json_encode($author));
    ...
    public function home_grafitti(Request $request)
    {
...
//         \Log::info(DB::getQueryLog());
    ...
    public function home_portraits(Request $request)
    {
