...
class ReturnGenerateSitemap extends Command
{
    protected $signature = 'sitemap:returned {SD}';
    ...
    public function handle()
    {
        ...
        // foreach (Settings::$mariaDBs as $site => $config) {

    //$this->info("Erstelle Sitemap für {$site}");

    // Hier deine Logik
        $currentSD = SD();
        // \Log::info("S: ".$currentSD);
        ...
        // === 4️⃣ Datei speichern ===
        $path = public_path("sitemap." . $currentSD . "_v3.xml");
        // $e("✅ Sitemap für Subdomain {$currentSD} erstellt: {$path}");
        return $xml->asXML();
    ...
    {
        $localhost = ["http://localhost/","http://test.mcs/",'http://pna.test.mcs',"http://chh.test.mcs","http://mfx.test.mcs","http://dag.test.mcs","http://ab.test.mcs"];
        $onliners = ["https://www.asario.de","https://www.ra-c-henning.de","https://www.monikadargies.de","https://www.marblefx.net","https://www.paulnadler.marblefx.net","https://www.marblefx.de"];
        $oldUrls = array_merge($localhost,$onliners);
        return str_replace(request()->getSchemeAndHttpHost(), "https://" . Settings::$dom[$sd], $url);
