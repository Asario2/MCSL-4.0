...
if (!function_exists('Notify')) {
    ...
    function Notify()
    {
        ...
        foreach ($entries as $entry) {
            ...
            if ($entry['type_ol'] === 'localhost' && str_contains($host, 'test.mcs')) {
                $lh = true;
            } elseif ($entry['type_ol'] === 'online' && !str_contains($host, 'test.mcs')) {
                $lh = true;
            } elseif ($entry['type_ol'] === 'both') {
                $lh = true;
            }
...
if (!function_exists('encval_user')) {
    /**
     * Verschlüsselt $value mit user-spezifischem Schlüssel.
     * Nur der jeweilige User kann entschlüsseln.
     */
    function encval_user($value, int $userId)
    {
        if ($value === null || $value === '') {
            return $value;
        }

        $userKey = user_key($userId);

        $encrypter = new Encrypter(base64_decode($userKey), 'AES-256-CBC');

        return $encrypter->encryptString((string) $value);
    }
}

if (!function_exists('decval_user')) {
    function decval_user($value, int $userId)
    {
        if ($value === null || $value === '') {
            return $value;
        }
        ...
        try {
            ...
            // Fallback: falls schon Klartext oder Schlüssel falsch
            return $value;
        }
    }
}
...
if(!function_exists('normalizePhone'))
{
    function normalizePhone($phone)
    {
        ...
        // +49 → 0 umwandeln (optional, falls gewünscht)
